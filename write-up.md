...
The pipeline successfully extracted aggregate `avg_rating` for all 20 deals (ranging from 4.0 to 4.9), pulled from a `data-testid="rating"` HTML element rendered by Playwright. However, individual review objects have `rating: null` across every single deal — per-review star scores are rendered dynamically after page load and are absent from the server-side HTML. Separately, none of the 20 deals expose an `AggregateRating` block in their JSON-LD schema — confirmed by checking `schema_types` across all 20 audit files. This means Google has no machine-readable rating signal for any deal page, and proposals recommending "add AggregateRating schema" are flagging a real structured-data gap, not a scraper limitation.
